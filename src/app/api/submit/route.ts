...
export async function POST(request: NextRequest) {
  ...
  const { data: created, error: dbError } = await supabase
    ...
    // The token is generated by a column default, so it has to come back from
    // the insert to build the tagged reply address below.
    .select("public_token")
    .single();
  ...
  // Replies to the confirmation land on the ticket's thread rather than in the
  // landlady's inbox. Falls back to her inbox if the receiving domain is unset,
  // so a missing variable can never bounce a tenant's reply.
  const tenantReplyTo =
    replyAddress(created.public_token, process.env.REPLY_DOMAIN) ?? REPLY_TO;
